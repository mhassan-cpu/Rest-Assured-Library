...
import static model.UpdateUserBody.getUpdateUserBody;
...
public class TC04_UpdateUser extends TestBase {
    ...
    @Test(priority = 1, description = "[3.2] check Update user positive flow")
    public void checkUpdateUser_P() {
        ...
        Assert.assertNotEquals(createdAt, updatedAt,
    ...
    @Test(priority = 2, description = "[3.2] check Update user negative scenarios")
    public void checkUpdateUser_N() {
        ...
        Response response = given()
                ...
                .header("Content-Type", "application/json")
                .header("g-token", "ROM831ESV")
                .body(getUpdateUserBody(invalidfirstname,invalidlastname,invalidemail))
                .when()
                .put("/users/" + userId)
                .then()
                .log().all()
